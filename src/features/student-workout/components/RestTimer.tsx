        ...
        <div className="space-y-3">
            ...
            <div className="flex shrink-0 gap-2">
              {isPaused ? (
                ...
                <button
                  type="button"
                  onClick={onPause}
                  className="rounded-full border border-[#E4DFD6] bg-[#FFFEFB] px-3 py-1 text-xs font-semibold text-[#2F4F3E] transition hover:bg-[#F3F0E8]"
                >
                  Pausar
                </button>
              )}
          ...
          {isPaused && (
            <p className="text-xs text-[#8A8378]">
              O timer está pausado e continuará do mesmo ponto ao retomar.
            </p>
          )}
        </div>
      ) : (
